...
static dwt_config_t config = {
    ...
    14,               /* TX preamble code. Used in TX only. */
    14,               /* RX preamble code. Used in RX only. */
...
static double rx_rmi_msg[RX_BUF_LEN];
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                	dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXFCE_BIT_MASK | SYS_STATUS_TXFRS_BIT_MASK);

                	frame_len = dwt_read32bitreg(RX_FINFO_ID) & FRAME_LEN_MAX_EX;
                	if (frame_len <= RX_BUF_LEN)
                	{
                		dwt_readrxdata(rx_rmi_msg, frame_len, 0);
                	}
                	double dist;
                	dist = rx_rmi_msg[10];
